...
| Main File  | `lab1_app/app.py`                                            |
| Database   | `lab1_app/requests.db` (auto-created at runtime)             |
...
Run the following commands inside the `lab1_app/` directory:
...
> **Note:** On first run, the SQLite database file `requests.db` is created automatically inside `lab1_app/`. No manual setup is required.
...
A: Delete `lab1_app/requests.db` and restart the app. The database will be re-initialized from scratch (all data will be lost).
